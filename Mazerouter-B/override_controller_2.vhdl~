...
ARCHITECTURE behavioural OF override_controller IS
	...
	SIGNAL override_next, override_reg : std_logic;
...
BEGIN
	...
	override_cont_new_state <= overide_cont_state; 
	pwm_count_reset <= '0';
	...
	CASE override_cont_state IS 
		WHEN read_sensor_and_listen => 	-- In deze staat zal hij lijnvolgen (dwz, overide = 0) totdat een bepaalde afstand is overschreden
			...
			IF (sensor_l = '0' AND sensor_m = '0' AND sensor_r = '0' AND 
			((distance_count > OPERATION_DISTANCE AND prev_operation_state = non_state)
		       	OR prev_operation_state = station)) THEN -- neem de lijnvolger over. 
	...
	-- Aan het einde van de actie komt de override_controller weer in zijn read_sensor_and_listen staat.
		...
		distance_count_reset <= '1'; -- Reset de beide tellers niet
			CASE station_state IS  
				WHEN first =>
					pwm_count_reset <= '0';
					override_next <= '0'; -- Gewoon lijnvolgen, hij zal automatisch recht door rijden!
...
BEGIN
	...
	CASE(tx_state_reg) IS
		WHEN tx_idle => -- Wachten wachten wachten wachten....
			...
			ElSIF(Distance_count > OPERATION_DISTANCE);
				tx_state_next <= tx_send;
			END IF;
		...
		WHEN tx_wait_normal => -- Forceer hem om maar een keer te sturen bij normale states. Aan het einde van een handeling wordt distance_count gereset
			tx_send_out <= '0';
			IF(override_cont_state = read_sensor_and_listen AND prev_operation_state = station) then 
				tx_state_next <= tx_idle;
			ELSIF(override_cont_state /= read_sensor_and_listen AND prev_operation_state = nonstation) 
				tx_state_next <= tx_idle;
			END IF;
	END CASE;
END PROCESS;
